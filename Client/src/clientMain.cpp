...
public:
    KeyboardListener(int id, std::mutex &mutex, bool &session, ConnectionHandler *ch) : _id(id), _mutex(mutex),
                                                                                        _session(session),
    ...
    void run() {
        while (_session) {
            ...
            if (line.compare("LOGOUT") == 0)
                std::this_thread::sleep_for(std::chrono::milliseconds(20)); // Wait here till "_session" will be equal to false.
        }
    }
};
